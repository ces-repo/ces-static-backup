...
Smart_Manager.prototype.saveView = function(action = 'create', name = '') {
    ...
    if(currentDashboardState){
        ...
        let isSaveDashboardAndCols = (jQuery('#sm_save_dashboard_and_cols').is(":checked")) ? true : false;
        let isSaveAdvancedSearch = (jQuery('#sm_save_advanced_search').is(":checked")) ? true : false;
        let params = {};
            ...
            params.data = {
                            ...
                            isSaveDashboardAndCols: isSaveDashboardAndCols,
                            isSaveAdvancedSearch: isSaveAdvancedSearch,
        ...
        window.smart_manager.send_request(params, function(response){
            ...
            if(ack == 'Success'){
                window.smart_manager.notification = {status:'success', message: sprintf(
                    ...
                    _x(`${((isSaveDashboardAndCols && isSaveAdvancedSearch) || (isSaveDashboardAndCols && !isSaveAdvancedSearch)) ? 'View' : 'Saved Search'} %sd successfully!`, 'notification', 'smart-manager-for-wp-e-commerce'), String(action).capitalize())}
		        if(viewSlug != ''){
                    setTimeout(function () {
...
Smart_Manager.prototype.createUpdateViewDialog = function(action = 'create', params = {}) {
    let viewSlug = window.smart_manager.getViewSlug(window.smart_manager.dashboardName);
...
Smart_Manager.prototype.deleteView = function( params={} ) {

    let viewSlug = (params.hasOwnProperty('view_slug') && params.view_slug.length) ? params.view_slug : window.smart_manager.getViewSlug(window.smart_manager.dashboardName);
    let message = (params.hasOwnProperty('success_msg') && params.success_msg.length) ? params.success_msg : 'View deleted successfully!';
    //AJAX to create & save view
    let ajaxParams = {};
        ajaxParams.data_type = 'json';
        ajaxParams.data = {
                        module: 'custom_views',
                        cmd: 'delete',
                        security: window.smart_manager.sm_nonce,
    ...
    window.smart_manager.send_request(ajaxParams, function(response){
        let ack = (response.hasOwnProperty('ACK')) ? response.ACK : ''
        if(ack == 'Success'){
            window.smart_manager.notification = {status:'success', message: _x(message, 'notification', 'smart-manager-for-wp-e-commerce')}
		    window.smart_manager.showNotification()
        }
        location.reload();
    });
...
Smart_Manager.prototype.handleSavedSearchConditions = function(type = {}){
    if(!type){
        return;
    }
...
Smart_Manager.prototype.checkPostParamsInSavedSearch = function ( savedSearchParams = {} ) {
...
Smart_Manager.prototype.GetEligibleDashboardsForSavedSearch = function ( SavedSearch = {} ) {
...
Smart_Manager.prototype.eligibleDashboardsDialog = function( eligibleDashboards = [] ) {
    if(!eligibleDashboards.length){
        return;
    }
    window.smart_manager.modal = {
    ...
    window.smart_manager.showModal();
    // Use MutationObserver to detect when the modal content is added to the DOM, when its loaded then init select2 on the content element.
    const observer = new MutationObserver((mutations, obs) => {
        if (jQuery('#eligible_dashboards_select').length) {
            jQuery('#eligible_dashboards_select').select2({
                dropdownCssClass: 'eligible-dashboards-select2-dropdown'
            });
...
Smart_Manager.prototype.getEligibleDashboardsHtml = function( eligibleDashboards = [] ) {
    return `
...
Smart_Manager.prototype.getPostsColumnsFromQuery = function( query = [] ) {
