 ...
 * - High season (May-September): Only large time windows available
...
jQuery(function($) {
    ...
    return dateObj >= closedStart && dateObj <= closedEnd;
}
...
function updateDeliveryOptions() {
    ...
    if (closedDates.length > 0) {
        ...
        // Warnung immer bei den Versanddetails einfügen
$('.shipping_method_description').first().after(message);
// Falls das nicht existiert, alternativ:
if (!$('.delivery-notice').length) {
    $('form.checkout').find('.woocommerce-shipping-fields, .shipping_method_description').first().prepend(message);
}
    ...
    // Fall 2: Hochsaison (Mai-September)
        ...
        // Info-Text anzeigen
        $('#lieferzeitraum_time').after('<p class="delivery-notice high-season" style="color: #ff6600;">In der Hochsaison (Mai-September) sind keine weiteren Zeiteingrenzungen verfügbar.</p>');
    }
    ...
    function setupEventListeners() {
        // Event-Listener für Änderungen an Datums- und Zeitfeldern
    ...
    $(document).ready(function() {
        ...
        $(document).ajaxComplete(function(event, xhr, settings) {
            if (settings.url && settings.url.indexOf('wc-ajax=update_order_review') !== -1) {
                ...
                setTimeout(function() {
                    if ($('#additional_date_from_liefer').length && $('#additional_date_to_liefer').length) {
                        updateDeliveryOptions();
                    }
                }, 500);
            }
